...
function above_text_taxonomy_edit_meta_field($term) {
  // put the term ID into a variable
  $t_id = $term->term_id;
  // retrieve the existing value(s) for this meta field. This returns an array
  $term_meta = get_term_meta($t_id,'after_content','');
  if(!$term_meta){ $term_meta = add_term_meta($t_id, 'after_content', '');}
   ?>
  <tr class="form-field">
  <th scope="row" valign="top"><label for="term_meta[cat_abovefooter]"><?php _e( 'After Content', 'flatsome' ); ?></label></th>
    <td>  
	<?php  
	       $content = isset($term_meta[0]) ? esc_attr( $term_meta[0] ) : '';
	       $settings = array('wpautop' => true, 'media_buttons' => true, 'quicktags' => true, 'textarea_rows' => '15', 'textarea_name' => 'cat_description1' );	
		   wp_editor(html_entity_decode($content , ENT_QUOTES, 'UTF-8'), 'cat_description1', $settings); 
    ?>	
      <p class="description"><?php _e( 'Enter a value for this field. Shortcodes are allowed. This will be displayed at bottom of the category.','flatsome' ); ?></p>
    </td>
  </tr>
...
add_action( 'product_cat_edit_form_fields', 'above_text_taxonomy_edit_meta_field', 10, 2 );
...
function save_taxonomy_custom_meta( $term_id ) {
	...
	if ( isset( $_POST['cat_description1'] )  )
		{
			 update_term_meta( $term_id, 'after_content', $_POST['cat_description1'] );
		}
